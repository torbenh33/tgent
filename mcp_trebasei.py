...
class RebaseEditorCoordinator:
    def __init__(self) -> None:
        self._incoming: asyncio.Queue[RebaseEditJob] = asyncio.Queue(maxsize=1)
        self._active_job: RebaseEditJob | None = None
        self._lock = asyncio.Lock()

    @staticmethod
    def _is_live(job: RebaseEditJob | None) -> bool:
        return job is not None and job.done_future is not None and not job.done_future.done()

    @staticmethod
    def _job_payload(job: RebaseEditJob) -> dict[str, Any]:
        return {
            "repo_path": job.repo_path,
            "todo_path": job.todo_path,
    ...
    async def enqueue_and_wait(
        ...
        async with self._lock:
            if self._is_live(self._active_job) or not self._incoming.empty():
                return {
                    "status": "error",
                    "message": "Another rebase editor job is already active.",
                }
        ...
        try:
            ...
            return {
                "status": "error",
                "message": "Timed out waiting for rebase todo edits. Git operation aborted.",
                "repo_path": job.repo_path,
            }

    async def _expire_job(self, job: RebaseEditJob) -> None:
        async with self._lock:
    ...
    async def submit_resolution(self) -> dict[str, Any]:
        ...
        if not self._is_live(job):
            return {"status": "error", "message": "No active rebase editor job."}

        try:
            with open(job.todo_path, "r", encoding="utf-8") as handle:
                handle.read()
        except OSError as err:
            return {
                "status": "error",
                "message": "Failed to read rebase todo file.",
                "error": str(err),
                "todo_path": job.todo_path,
            }
...
async def rebasei_status() -> dict[str, Any]:
    """Return current in-memory rebase editor status."""
    queue_status = await COORDINATOR.status()
    queue_status["active_runs"] = WORKFLOW_RUNNER.list_runs("rebasei")
    return queue_status
